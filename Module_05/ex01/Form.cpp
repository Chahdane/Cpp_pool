...
Form::Form(std::string name, int sign_g, int execute_g) : name(name), grade_to_sign(sign_g), grade_to_execute(execute_g)
{
...
{
    if (bureacrat.getGrade() <= this->get_grade_to_sign())
    {
        ...
        std::cout << name << " got signed by: " << bureacrat.getName() << std::endl;
        this->is_signed = true;
    }
    else
        throw GradeTooLowException();
